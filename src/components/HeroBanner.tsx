        ...
        <div className='w-full h-full flex items-end p-2 lg:p-10 bg-opacity-60 bg-slate-200'>
            <div className='ml-2 xl:ml-44 w-1/2'>
                ...
                <div className='flex items-center'>
                 <Image height={20} width={20} src={'/assets/dummy/fakeProfile.jpeg'} className='h-10 w-10 mr-3 border-2 border-white rounded-[50%]' alt='new'/>
                 <span>Warzone, US English</span>
                </div>
                <button className='my-8 bg-violet-600 font-bold px-6 py-2 rounded-lg hover:bg-violet-800'>Watch</button>
            </div>
            <div className='h-44 w-full flex items-center'>
              {dummyLives.map((live, index) => <Link href={'/nv'}>
                <Image height={30} width={30} src={'/assets/dummy/thumbnail.jpeg'} className='h-20 w-32 object-cover mx-4 rounded-lg' alt='live thumbnail'/>
              </Link>)}
            </div>
        </div>
    </div>
